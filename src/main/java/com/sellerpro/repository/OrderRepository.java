...
public interface OrderRepository extends JpaRepository<Order, Long> {
    ...
    @Query("SELECT COUNT(o) FROM Order o WHERE o.client.user.email = :email AND o.orderDate = CURRENT_DATE")
    Long countTodayOrders(@Param("email") String email);

    @Query("SELECT SUM(o.netRevenue) FROM Order o WHERE o.client.user.email = :email AND o.orderDate = CURRENT_DATE")
    BigDecimal sumTodayRevenue(@Param("email") String email);
    ...
    @Query("SELECT FUNCTION('DATE_FORMAT', o.orderDate, '%Y-%m'), o.platform, COUNT(o), SUM(o.netRevenue) " +
           "FROM Order o WHERE o.client.user.email = :email AND o.platform = :platform AND o.orderDate >= :from " +
           "GROUP BY FUNCTION('DATE_FORMAT', o.orderDate, '%Y-%m'), o.platform")
    List<Object[]> findMonthlyTrendByPlatform(@Param("email") String email, @Param("platform") String platform, @Param("from") LocalDate from);

    default List<Object[]> findMonthlyTrendByPlatform(String email, String platform, int months) {
        return findMonthlyTrendByPlatform(email, platform, LocalDate.now().minusMonths(months));
    }
    ...
    @Query("SELECT o.sku, o.productName, SUM(o.quantity), SUM(o.netRevenue) FROM Order o " +
           "WHERE o.client.user.email = :email AND o.orderDate >= :from " +
           "GROUP BY o.sku, o.productName ORDER BY SUM(o.netRevenue) DESC")
    List<Object[]> findTopSkusFull(@Param("email") String email, @Param("from") LocalDate from);

    default List<Object[]> findTopSkus(String email, int limit) {
        return findTopSkusFull(email, LocalDate.now().minusMonths(3))
            .stream().limit(limit).collect(java.util.stream.Collectors.toList());
    }

    @Query("SELECT o.sku, o.productName, SUM(o.quantity), SUM(o.netRevenue) FROM Order o " +
           "WHERE o.client.user.email = :email AND o.platform = :platform AND o.orderDate >= :from " +
           "GROUP BY o.sku, o.productName ORDER BY SUM(o.netRevenue) DESC")
    List<Object[]> findTopSkusByPlatformFull(@Param("email") String email, @Param("platform") String platform, @Param("from") LocalDate from);

    default List<Object[]> findTopSkusByPlatform(String email, String platform, int limit) {
        return findTopSkusByPlatformFull(email, platform, LocalDate.now().minusMonths(3))
            .stream().limit(limit).collect(java.util.stream.Collectors.toList());
    }

    @Query("SELECT COUNT(o), SUM(o.netRevenue), AVG(o.netRevenue) FROM Order o WHERE o.client.user.email = :email AND o.platform = 'WEBSITE'")
